...
['juan', 'andrea', 'leon', 'andres', 'natalia', 'camilo', 'rusbel', 'johan'].each do |name|
  ...
  ['desarrollo', 'mercadeo', 'conceptualización', 'ejercicios'].each do |category_name|
    Category.create(
      name:        category_name,
      description: '--' 
    )
  ...
  # 3. Referencia al propietario de las tareas
  owner = User.find_by(email: 'johan@platzi.com')
  
  # 4. Definición de las tareas y sus participantes
  [
  ...
  ].each do |category_name, task_description, participant_set|
  
    # Construimos los participantes
